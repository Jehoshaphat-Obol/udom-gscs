...
export default class StudentDashboard extends Component {
    ...
    updateStatus(){
        axios.put(apiUrl + 'student_update_status/', {graduation_status: this.state.graduation_status})
        .then(response=>{
            this.setState(prevState => ({
                ...prevState,
                graduation_status: response.data.graduation_status,
                status: (response.data.graduation_status == "EX")?("Expected"):("Postponed"),
            }))
        })
        .catch(error=>{
            console.log(error)
    ...
    reloadStatus(){
        axios.get(apiUrl + 'student_update_status/')
        .then(response=>{
            this.setState(prevState => ({
                ...prevState,
                graduation_status: response.data.graduation_status,
                        ...
                        <div className="col-sm-12 col-md-8">
                            <div className="card">
                                <div className="card-body">
                                    ...
                                    <p><b>College:</b> {currentUser.college}</p>
                                    <p><b>Degree:</b> {currentUser.degree_level}</p>
                                    <p><b>Programme:</b> {currentUser.degree_program}</p>
                                    <p><b>Graduation status:</b><span className={(status == "Expected")?('rounded p-1 bg-success text-white'):('rounded p-1 bg-danger text-white')}>{status}</span></p>
                                </div>
                            </div>
                            <div className="card">
                                <div className="card-body">
